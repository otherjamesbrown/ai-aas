...
Follow this 8-step workflow to create a feature specification:
...
If exists, load:
- Project name and beads prefix
- Path conventions
- Validation template for Step 6
...
- **Metadata**: Feature branch, created date (YYYY-MM-DD), status, input description
...
- **Validation**: Environment-specific checks (see Step 6)

### Step 6: Add Validation Section

Append a Validation section to spec.md for post-deployment testing.

If project config exists, use `validation_template` from config. Otherwise use:
...
Customize the feature-specific checks based on the requirements.
...
- Spec path
- Readiness status for next phase (`/speckit.plan` or `/speckit.impact` if migration)
...
═══════════════════════════════════════════════════
 ...
 Next Steps:
   - If migration/refactor: /speckit.impact
   - If greenfield: /speckit.plan
═══════════════════════════════════════════════════
```
