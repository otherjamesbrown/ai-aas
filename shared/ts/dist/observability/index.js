...
export async function startTelemetry(config) {
...
async function startTelemetryInternal(config, allowHttpFallback) {
    const exporter = config.protocol === 'http'
    ...
    try {
        ...
        diag.error('Failed to start telemetry exporter', error);
        incrementTelemetryExporterFailure(config.serviceName, config.protocol);
