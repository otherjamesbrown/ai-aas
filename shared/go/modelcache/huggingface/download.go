...
type DownloadOptions struct {
	...
	// OnFileStart is called when a file download starts (for progress tracking)
	OnFileStart func(filename string)
	// OnFileComplete is called when a file download completes
	OnFileComplete func(filename string, size int64)
	// CheckCancelled is called periodically to check if the operation should be cancelled
...
func (c *Client) DownloadModel(ctx context.Context, modelID string, opts DownloadOptions) (*DownloadResult, error) {
	...
	for _, f := range downloadFiles {
		...
		go func(file RepoFile) {
			...
			atomic.AddInt64(&downloadedBytes, downloaded.Size)
			if opts.OnProgress != nil {
				opts.OnProgress(atomic.LoadInt64(&downloadedBytes), totalSize)
			}

			if opts.OnFileComplete != nil {
				opts.OnFileComplete(file.Path, downloaded.Size)
			}
		}(f)
	}
