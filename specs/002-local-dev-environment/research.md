...
| Secrets distribution | Use GitHub repository environment secrets accessed via a fine-grained PAT; `make secrets-sync` calls a Go helper (`cmd/secrets-sync`) that reads secrets with `gh api` and writes `.env.linode` & `.env.local` with masked values | Reuses existing GitHub secrets management, keeps credentials centralized, supports TTL via PAT rotation | 1Password CLI (does not integrate with automation/TTL), storing secrets in Linode metadata (security risk) |
...
- Confirm GitHub environment secrets exist for remote/local dev (`DEV_REMOTE_*`, `DEV_LOCAL_*`) and that developers obtain a PAT with `actions:read` scope before wiring `make secrets-sync`.
