...
Examples:
  ...
  ai-aas-cli model logs llama-3-8b -e development -f
...
Examples:
  ...
  ai-aas-cli model test llama-3-8b -e development --timeout 60s`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !isvc.Ready {
				return fmt.Errorf("model not ready (conditions may indicate the issue)")
			}
	...
	cmd.Flags().StringVarP(&environment, "environment", "e", "", "target environment (required)")
	cmd.Flags().StringVarP(&prompt, "prompt", "p", "Hello, how are you?", "test prompt")
