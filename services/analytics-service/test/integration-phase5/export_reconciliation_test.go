...
// mockS3Delivery wraps S3Delivery for testing, storing uploads in memory.
// Note: This is a workaround since S3Delivery is a concrete type, not an interface.
// In production, we'd use a real S3-compatible storage or a test S3 server.
type mockS3Delivery struct {
	*exports.S3Delivery
	uploads map[string][]byte // key -> CSV data
}

func newMockS3Delivery(logger *zap.Logger) *mockS3Delivery {
	// Create a minimal S3Delivery instance for testing
	// We'll override the UploadCSV method behavior via the mock
	s3Delivery, _ := exports.NewS3Delivery("", "test", "test", "test-bucket", "us-east-1", 24*time.Hour, logger)
...
func (m *mockS3Delivery) UploadCSV(ctx context.Context, orgID, jobID uuid.UUID, csvData []byte) (string, string, error) {
	...
	// Calculate actual SHA-256 checksum for validation
	checksum := calculateSHA256(csvData)
...
func TestCSVReconciliation(t *testing.T) {
	...
	obsCfg := observability.Config{
		...
		Environment: "test",
		Endpoint:    "http://localhost:4318", // Dummy endpoint for testing
		Protocol:    "http",
		LogLevel:    "info",
	}
	...
	mockS3 := newMockS3Delivery(obs.Logger)
	...
	jobRunner := exports.NewJobRunner(exports.RunnerConfig{
		Pool:       pool,
		S3Delivery: mockS3.S3Delivery, // Use embedded S3Delivery
		Logger:     obs.Logger,
