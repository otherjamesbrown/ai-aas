...
**Status**: Mostly Complete - Core authentication, MFA enforcement, API key lifecycle, IdP federation stubs, and recovery endpoints ready  
**Next Action**: Complete OIDC token issuance, email integration, admin approval workflows, and end-to-end tests
...
### T008: Authentication Service ✅ Complete
- **Status**: Complete
- **Completed**:
  ...
  - ✅ **IdP federation stubs** (`go-oidc` integration for external identity providers)
    - ✅ OIDC provider configuration structure
    - ✅ `/v1/auth/oidc/{provider}/login` endpoint
    - ✅ `/v1/auth/oidc/{provider}/callback` endpoint
    - ✅ External IdP user mapping via `external_idp_id`
- **Key Files**:
...
### T009: User/Org Lifecycle Handlers ✅ Complete
- **Status**: Complete
- **Completed**:
  ...
  - ✅ **Recovery API endpoints**:
    - ✅ `POST /v1/auth/recover` - Initiate recovery (generate recovery token)
    - ✅ `POST /v1/auth/recover/verify` - Verify recovery token validity
    - ✅ `POST /v1/auth/recover/reset` - Reset password with recovery token
    - ✅ Recovery token generation and validation
    - ✅ Recovery token storage in `recovery_tokens` JSONB array
- **Key Files**:
...
- `github.com/coreos/go-oidc/v3 v3.16.0` - **NEW** OIDC client library for IdP federation
- `golang.org/x/oauth2` - OAuth2 client library (transitive dependency of go-oidc)
...
- Add audit trails for recovery operations (partially complete - basic audit events added)
...
### Authentication & MFA
- `internal/httpapi/auth/handlers.go` - Login handler with MFA enforcement, recovery routes
- `internal/httpapi/auth/oidc.go` - **NEW** OIDC/IdP federation handlers
- `internal/httpapi/auth/recovery.go` - **NEW** Password recovery endpoints
- `internal/security/mfa.go` - **NEW** TOTP verification utilities
- `internal/oauth/store.go` - OAuth2 storage with Redis caching
- `internal/oauth/provider.go` - Fosite provider composition
- `internal/storage/postgres/store.go` - Added org membership validation methods, GetServiceAccountByID, UpdateUserRecoveryTokens
...
1. **IdP Federation Stub**: IdP providers must be configured via code (not yet configurable via env vars). OIDC callback flow issues tokens but doesn't complete full OAuth2 session creation yet.
2. **Recovery Token Storage**: Recovery tokens are stored in JSONB array (works but not optimized for large-scale queries). Consider separate recovery_tokens table for production.
...
6. **Recovery Email**: Recovery tokens are returned in response in dev mode but email sending not implemented (production requires email service integration)
...
- ⏳ T011: Credential recovery and lockout workflows (admin approval pending)
...
1. Complete OIDC callback flow to issue proper OAuth2 tokens (currently redirects without tokens)
2. Add IdP provider configuration via environment variables
3. Implement email service integration for recovery token delivery
4. Add admin approval workflow for recovery requests (T011)
5. Create end-to-end tests (T012)
