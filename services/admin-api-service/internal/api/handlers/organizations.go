...
func (h *OrganizationHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if _, ok := err.(*service.ConflictError); ok {
			api.WriteConflict(w, r, err.Error())
			return
		}
...
func (h *OrganizationHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if org == nil {
		api.WriteNotFound(w, r, "Organization", orgIDStr)
		return
	}
...
func (h *OrganizationHandler) validateCreate(create *domain.OrganizationCreate) []api.ValidationError {
	var errors []api.ValidationError

	if create.Slug == "" {
		errors = append(errors, api.ValidationError{Field: "slug", Message: "slug is required"})
	} else if len(create.Slug) > 100 {
		errors = append(errors, api.ValidationError{Field: "slug", Message: "slug must be 100 characters or less"})
	} else if !slugRegex.MatchString(create.Slug) {
		errors = append(errors, api.ValidationError{Field: "slug", Message: "slug must start with a letter and contain only lowercase letters, numbers, and hyphens"})
	}

	if create.DisplayName == "" {
		errors = append(errors, api.ValidationError{Field: "display_name", Message: "display_name is required"})
	} else if len(create.DisplayName) > 255 {
		errors = append(errors, api.ValidationError{Field: "display_name", Message: "display_name must be 255 characters or less"})
	}

	if create.PlanTier != "" && !domain.IsValidPlanTier(create.PlanTier) {
		errors = append(errors, api.ValidationError{Field: "plan_tier", Message: "Invalid plan_tier. Must be one of: free, starter, enterprise"})
	}

	return errors
}

func (h *OrganizationHandler) validateUpdate(update *domain.OrganizationUpdate) []api.ValidationError {
	var errors []api.ValidationError
	...
	if update.Status != nil && !domain.IsValidOrgStatus(*update.Status) {
		errors = append(errors, api.ValidationError{Field: "status", Message: "Invalid status. Must be one of: active, suspended, deleted"})
	}
