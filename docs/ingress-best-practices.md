...
Development environments use public DNS with Let's Encrypt certificates.
...
All environments use cert-manager with Let's Encrypt for automated certificate management:
...
```bash
curl https://api.dev.otherjamesbrown.com/v1/status/healthz \
...
- [ ] Configure hostname for the environment (e.g., `service.dev.otherjamesbrown.com`)
