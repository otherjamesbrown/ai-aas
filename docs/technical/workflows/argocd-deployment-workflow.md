...
- ArgoCD syncs Helm charts and manifests, and applies changes
...
# For models
kubectl get inferenceservice -n system
```
...
kubectl get application kserve-llama-2-7b-development -n argocd -o yaml | grep -A 10 "status:"
```

**Check if watching correct branch:**
```bash
kubectl get application kserve-llama-2-7b-development -n argocd -o jsonpath='{.spec.source.targetRevision}'
# Should be: main
```

**Check current sync revision:**
```bash
kubectl get application kserve-llama-2-7b-development -n argocd -o jsonpath='{.status.sync.revision}'
...
```bash
# Via kubectl
kubectl patch application kserve-llama-2-7b-development -n argocd \
...
1. **Always update source files first**
   - Helm values for services.
   - KServe manifests for models.
...
**Check sync status:**
```bash
kubectl get application kserve-llama-2-7b-development -n argocd -o jsonpath='{.status.sync.status}'
```
...
- `docs/technical/workflows/deploy-to-environments.md` - Complete deployment guide
