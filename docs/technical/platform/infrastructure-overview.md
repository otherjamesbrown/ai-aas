...
- **Node Pools**:
  ...
  - GPU (system): `g1-gpu-rtx6000` (2 nodes) reserved for KServe InferenceServices workloads.
- **Networking**:
  - Calico NetworkPolicies implement default-deny stance.
  - Ingress via NGINX Ingress Controller for general traffic and Istio Service Mesh for model serving traffic.
  - cert-manager (Let's Encrypt for production) for TLS.
...
- **GitOps**:
  ...
  - ArgoCD reconciles Helm charts (`infra/helm/charts/*`), including observability, ingress, and KServe components.
...
- `docs/technical/platform/linode-access.md`: Token creation and CLI setup.
- `docs/technical/platform/access-control.md`: Detailed RBAC and package issuance procedures.
- `docs/technical/platform/observability-guide.md`: Dashboard conventions, alert tuning.
- `docs/technical/platform/endpoints-and-urls.md`: Complete endpoint and URL configuration guide.
- `docs/technical/platform/tls-ssl-setup.md`: TLS/SSL certificate setup and management.
- `docs/technical/workflows/infrastructure-rollback.md`: Step-by-step rollback instructions.
- `docs/technical/workflows/infrastructure-troubleshooting.md`: Issue resolution catalog.
- `infra/secrets/certs/README.md`: Self-signed certificate setup and management.

Keep this document updated alongside infrastructure changes to ensure context for downstream specs (`005-user-org-service`, `011-observability`, `016-kserve-migration`).
